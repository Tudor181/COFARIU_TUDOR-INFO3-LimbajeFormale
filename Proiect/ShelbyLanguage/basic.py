...
class RTError(Error):
    ...
    def generate_traceback(self):
        ...
        while ctx:
            result = (
                f"  File {pos.file_name}, line {str(pos.ln + 1)}, in {ctx.display_name}\n"
                + result
            )
...
class Position:
    def __init__(self, index: int, ln, col, file_name, file_text):
        self.index = index
        self.ln = ln
        self.col = col
...
#######################################
## TT - token type
##@check - poate putem adauga mai multe tipuri de tokens verifica 3.3 din pdf
TT_INT = "INT"
TT_FLOAT = "FLOAT"
TT_STRING = "STRING"
...
class Token:
    ...
    def __repr__(self):
        """Returnează o reprezentare ușor de citit a obiectului Token,
        folosită automat de funcția print() și consola Python.
        Exemplu: Token(TT_INT, 5) -> 'INT:5'"""
...
class Lexer:
    def __init__(self, file_name, text):
        self.file_name = file_name
        self.text = text
        self.pos = Position(index=-1, ln=0, col=-1, file_name=file_name, file_text=text)
        ...
        while self.current_char != None:
            if self.current_char in " \t":
                ...
                return [], IllegalCharError(
                    pos_start, self.pos, details="'" + char + "'"
                )
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_run(self, exec_ctx):
        ...
        try:
            # with keyword is safer and newer than try except (auto calls close etc)
            with open(fn, "r") as f:
                script = f.read()
...
# INTERPRETER
#######################################
type Node = StringNode | ListNode  # etc


class Interpreter:
    def visit(self, node: Node, context):
...
###
# FOR (ONE LINE NOT MULTILINE) EXAMPLE:
# VAR b = 0
# FOR i = 1 TO 5 STEP 2 THEN VAR b = b + i
# the second var is that the parser only allows assigning values if we have VAR in front of them (check VarAssignNode)


#!!!Steps pt ca PRINT("TEST") sa functioneze:
# 1. Lexer - tokenizare - PRINT - identifier, ( - TT_LPAREN , "TEST" -> string - lexer vede " si apeleaza _make_string, ) - TT_RPAREN
# 2. Parser - .expr() , .call() si merge pana la .atom() unde gaseste un IDENTIFIER atunci va returna un VarAccessNode in .call() = node_to_call, apoi vede ca sunt parantezele corecte, si seteaza si argumente daca sunt, poate cu VarAccesNode sau StringNode sau NumberNode etc.
